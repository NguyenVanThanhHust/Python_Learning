...
for room in range(1, 26): # add romdom scenery in planet locations
...
for room_coordinate in range(0, 13):
    for room_number in [1, 2, 3, 4, 5]: # add the top fence
        scenery[room_number] += [31, 0, room_coordinate]
    for room_number in [1, 6, 11, 16, 21]: # add the left fence
        scenery[room_number] += [31, room_coordinate, 0]
    for room_number in [5, 10, 15, 20, 25]: # add the right fence
